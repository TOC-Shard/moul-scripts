...
sdlCurrentSongVar = 'Event16'

class codDisco(ptModifier):
    ...
    def __init__(self):
        ...
        self.id = 8501003
        self.version = 1
        PtDebugPrint(('codDisco: init  version = %d' % self.version))
    ...
    def OnServerInitComplete(self):
        ...
        if (not (os.access(strPath.value, os.F_OK))):
            PtDebugPrint('codDisco:ERROR! simpleImager: Couldn\'t find the directory...creating', level=kErrorLevel)
            os.mkdir(strPath.value)
        SoundObjIndex = soSoundObj.value.getSoundIndex(strSoundObj.value)
        PtDebugPrint(('codDisco.OnServerInitComplete: using sound object index:' + str(SoundObjIndex)))
    ...
    def PlaySong(self, filename):
        ...
        if (filename[-4:].lower() == '.ogg'):
            ...
            iscompressed = 0
        PtDebugPrint(('codDisco.PlaySong: Going to play: %s' % filename))
    ...
    def NextSong(self):
        ...
        if (curIndex >= 0):
            ...
            PtDebugPrint('codDisco.NextSong: Stopping')
            CurrentFile = None
            ageSDL = PtGetAgeSDL()
